...
const props = defineProps<{ strandId: string; path: InstrPath; instruction: Extract<InstructionDto, { type: 'CloseApp' }> }>();
...
<template>
  <span class="instruction-label">Close:</span>
  ...
  <AppSelectorDialog v-if="showDialog" title="Choose an App to Close" @select="onSelect" @close="showDialog = false" />
</template>
