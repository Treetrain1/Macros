...
const props = defineProps<{ strandId: string; path: InstrPath; instruction: Extract<InstructionDto, { type: 'SetVariable' }> }>();
...
<template>
  ...
  <ValueBlock :location="fieldLocation(strandId, path, 'SetVariableValue')" :value="instruction.value" />
</template>
