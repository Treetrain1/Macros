...
function onModeChange(v: string) {
  commit(withScheduleMode(props.instruction.schedule, v as TimeScheduleDto['kind']));
}

function onWeekdayChange(v: string) {
  const s = props.instruction.schedule;
  if (s.kind !== 'Weekly') return;
  commit({ ...s, weekday: v as WeekdayDto });
}
...
function onDayChange(e: Event) {
  ...
  commit({ ...s, day: clampDay((e.target as HTMLInputElement).value) });
}

function onMonthChange(v: string) {
  const s = props.instruction.schedule;
  if (s.kind !== 'Yearly') return;
  commit({ ...s, month: Number(v) });
}

function onTimeChange(e: Event) {
  commit(withTimeValue(props.instruction.schedule, (e.target as HTMLInputElement).value));
}
